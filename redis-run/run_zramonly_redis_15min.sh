...
sudo ./bin/ycsb load redis -s -P workloads/workload_redis_15min -p "redis.host=127.0.0.1" -p "redis.port=6379" > $scriptdir/outputLoad.txt 2>&1
sleep 5
sudo ./bin/ycsb run redis -s -P workloads/workload_redis_15min -p "redis.host=127.0.0.1" -p "redis.port=6379" > $scriptdir/outputRun-$zramcapicity-$memcapicity-$postfix 2>&1
