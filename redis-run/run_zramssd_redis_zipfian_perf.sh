...
sleep 3
redispid=$(cat /sys/fs/cgroup/$cgroup_name/cgroup.procs)
kswapdpid=$(ps -e | grep kswapd | awk '{print $1}')

# perf record -F 99 -g -p $kswapdpid &
perf record -F 99 -g -p $redispid &
perf_pid=$!
